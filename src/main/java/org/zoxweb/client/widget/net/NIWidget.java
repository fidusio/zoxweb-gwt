...
implements SetValue<NIConfigDAO>
{
    ...
    public NIWidget() {
        ...
        lbcNI = new ListBoxController<String>(lbNetwokInterface, "wan", "lan")
        {
...
//  }
    ...
    public void setInfo(NIConfigDAO nicd)
    {
        ...
        if (nicd != null)
        {
            NVGenericMap nvgm = nicd.getProperties();
            GetNameValue<?>[] values = nvgm.values();
            gInfo.resize(values.length, 2); 
            for (int r = 0; r < values.length; r++)
            {
                
                
                gInfo.setWidget(r, 0, new Label(values[r].getName()));
                TextBox tb = new TextBox();
                tb.setEnabled(false);
                tb.setWidth(WIDTH);
                
                //GWT quirk for generic values
                String val = ""+values[r].getValue();
                tb.setValue(val);
                
                gInfo.setWidget(r, 1,tb);
            }
        }
